...
# Run test suite
./target/release/caro test --backend static --suite .Codex/beta-testing/test-cases.yaml > /tmp/cycle-N-results.txt
...
```bash
# Stage changes
git add src/backends/static_matcher.rs .Codex/beta-testing/cycles/cycle-N-*.md
...
1. **Prepare**: Ensure test suite exists at `.Codex/beta-testing/test-cases.yaml`
