...
`remotion-best-practices` skill at `~/.Codex/skills/remotion-best-practices/`.
...
1. **Never invent commands.** Pull queries + expected outputs from
   `.Codex/beta-testing/test-cases.yaml`. If you need a query that isn't
...
Scene 2 queries (verified against `.Codex/beta-testing/test-cases.yaml`):
...
1. Re-run each Scene 2 query against the current `caro` binary; if any
   command drifted, update `.Codex/beta-testing/test-cases.yaml` first
...
| `.Codex/beta-testing/test-cases.yaml` | Verified caro queries + outputs | Scene 2 |
...
This skill is **agent-claimable**. A Codex session reaching this
section should:
