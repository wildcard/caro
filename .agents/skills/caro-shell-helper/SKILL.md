...
description: "DEPRECATED 2026-05-16 — use caro-shell instead. This skill recommends the --backend Codex flag and a ~/.config/caro/config.toml path that do not work on the current caro 1.4.0 binary, and its 522-line educational body is 4× the size of caro-shell for no benefit to an agent. Will be removed after 2026-08-01."
version: "1.0.0"
...
## Default Backend: Codex Haiku 4.5

**IMPORTANT**: When this skill is used within Codex, always use the Codex backend by default. This provides the fastest response times and best accuracy.

```bash
# Default command format when running in Codex
caro --backend Codex "user's request here"
```

The Codex backend uses **Codex-haiku-4-5-20251101** for fast, cost-effective command generation. The API key is automatically provided by Codex.
...
When the user describes what they need, use Caro with the Codex backend (default for Codex):

```bash
# Always use --backend Codex when running in Codex
caro --backend Codex "user's natural language description"
```

**Example:**
```bash
$ caro --backend Codex "find all PDF files larger than 10MB in Downloads"

Generated command:
  find ~/Downloads -name "*.pdf" -size +10M -ls

Safety Assessment: ✅ Safe (Green)
Backend: Codex (Codex-haiku-4-5-20251101)
...
### Codex Backend (Default for Codex)
- **Best for**: Running directly within Codex or with Anthropic API access
- **Advantages**: Fastest, most accurate, uses Codex Haiku 4.5 by default
- **Model**: Codex-haiku-4-5-20251101 (default)
- **Setup**:
  ```toml
  ...
  [backend.Codex]
  model_name = "Codex-haiku-4-5-20251101"  # Fast and cost-effective
  # API key read from ANTHROPIC_API_KEY environment variable
  ```

**When running in Codex**: The Codex backend is automatically preferred as it provides the best integration and fastest response times. No additional configuration is needed - Codex provides the API key automatically.
...
**Response:**
```
I'll help you generate a safe command using Caro with Codex Haiku 4.5.

$ caro --backend Codex "find all Python files modified in the last week"
...
**Response:**
```
This involves deletion, so let's be careful. I'll use Caro with Codex Haiku 4.5.

$ caro --backend Codex "delete all .log files older than 30 days"
...
**Response:**
```
$ caro --backend Codex "check disk space for all mounted filesystems"
...
**Response:**
```
This is a multi-step operation. Let's do it safely using Codex Haiku 4.5:

Step 1: Find large files (>100MB)
$ caro --backend Codex "find files larger than 100MB"
...
$ caro --backend Codex "compress file.txt with gzip"
  gzip -k file.txt
...
[backend]
primary = "Codex"  # Codex (default in Codex), embedded, ollama, or vllm
...
caro --backend Codex "your prompt here"   # Use Codex Haiku 4.5 (fastest)
...
# When running in Codex, the API key is provided automatically
```
