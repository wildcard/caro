...
### Tier 2 — Prune `.Codex/worktrees/` (~15–18 GB)

These are ephemeral session worktrees from Codex's worktree integration.
Most are named `Codex/<adjective-name-hash>` and tied to merged or pushed
...
takes the worktree root (`.Codex/worktrees` or `.worktrees`) as `$1` and
writes two files:
...
# Tier 2
bash <(cat .Codex/skills/caro.prune/audit_wts.sh) .Codex/worktrees
...
# Tier 3
bash <(cat .Codex/skills/caro.prune/audit_wts.sh) .worktrees
...
| 2 (.Codex/worktrees) | 10–18 GB | ~3 min | Low |
...
- `~/.Codex/rules/destructive-commands.md` — the rule this skill operates under
- `.Codex/rules/git-workflow.md` — never run on `main` branch
...
- The 2026-05-16 cleanup plan: `~/.Codex/plans/let-s-analze-the-space-majestic-squid.md`
