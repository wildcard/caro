...
Run the `~/.Codex/scripts/spec_metadata.sh` script to generate all relevant metadata
...
```bash
cat ~/.Codex/state/braintrust_sessions/*.json | jq -s 'sort_by(.started) | last'
```
...
HANDOFF_ID=$(sqlite3 .Codex/cache/artifact-index/context.db "SELECT id FROM handoffs ORDER BY indexed_at DESC LIMIT 1")
