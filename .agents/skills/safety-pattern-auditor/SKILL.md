...
- [ ] Test suite available: `.Codex/beta-testing/test-cases.yaml`
...
1. **Capture pattern metrics**:
   ```bash
   ./scripts/capture-safety-baseline.sh .Codex/audits/baseline-$(date +%Y%m%d).json
   ```

2. **Review baseline**:
   ```bash
   cat .Codex/audits/baseline-$(date +%Y%m%d).json | python3 -m json.tool
   ```
...
1. **Run gap analyzer**:
   ...
   ./scripts/analyze-pattern-gaps.py src/safety/patterns.rs \
     -o .Codex/audits/gaps-$(date +%Y%m%d).md
   ```

2. **Review gap report**:
   ```bash
   cat .Codex/audits/gaps-$(date +%Y%m%d).md
   ```

3. **Categorize gaps by detector type**:
   ```bash
   # Count gaps by type
   grep "**Type**: argument_order" .Codex/audits/gaps-*.md | wc -l
   grep "**Type**: path_variant" .Codex/audits/gaps-*.md | wc -l
   grep "**Type**: wildcard" .Codex/audits/gaps-*.md | wc -l
   grep "**Type**: platform" .Codex/audits/gaps-*.md | wc -l
   ```
...
1. **Run regression checker**:
   ```bash
   ./scripts/check-safety-regressions.sh .Codex/audits/baseline-*.json
   ```

2. **Run full test suite**:
   ```bash
   ./target/release/caro test --backend static \
     --suite .Codex/beta-testing/test-cases.yaml
   ```
...
5. **Save audit artifacts**:
   ```bash
   mkdir -p .Codex/audits/2026-01-08/
   mv .Codex/audits/baseline-*.json .Codex/audits/2026-01-08/
   mv .Codex/audits/gaps-*.md .Codex/audits/2026-01-08/
   cp priority-matrix.md .Codex/audits/2026-01-08/
   ```
...
- Pattern Developer Skill: `.Codex/skills/safety-pattern-developer/`

**If stuck**:
1. Review `examples/audit-report-template.md`
2. Check previous audit artifacts in `.Codex/audits/`
