...
3. **Document the threat**:
   Create a threat document in `.Codex/beta-testing/threats/`
...
1. **Create test file**:
   ```bash
   .Codex/beta-testing/pattern-[descriptive-name]-test.yaml
   ```
...
3. **Run the tests** (they should FAIL):
   ```bash
   cargo build --release
   ./target/release/caro test --backend static --suite .Codex/beta-testing/pattern-parent-deletion-test.yaml
   ```
...
2. **Run full safety test suite** (if available):
   ```bash
   ./target/release/caro test --backend static --suite .Codex/beta-testing/test-cases.yaml
   ```
...
3. **Create commit**:
   ...
   git add .Codex/beta-testing/pattern-parent-deletion-test.yaml
...
**If stuck**:
1. Review examples in `.Codex/skills/safety-pattern-developer/examples/`
...
- Contribution Guide: `.Codex/workflows/add-safety-pattern.md`
