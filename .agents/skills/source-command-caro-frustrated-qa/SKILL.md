---
name: "source-command-caro-frustrated-qa"
description: "Migrated source command `caro.frustrated-qa`"
---

# source-command-caro-frustrated-qa

Use this skill when the user asks to run the migrated source command `caro.frustrated-qa`.
...
Wired into `.Codex/automation/config/schedule.yaml` as
...
# The harness only loads .Codex/agents/*.md at session start. If the
...
test -f .Codex/agents/caro-frustrated-beta.md || {
  echo "PREFLIGHT_FAIL: persona agent file missing at .Codex/agents/caro-frustrated-beta.md"
  exit 3
}
...
RUN_DIR=".Codex/beta-testing/runs/$(date +%Y-%m-%d)"
...
comment template (`~/.Codex/rules/pr-comment-structure.md`):

```
`[agent]`

**Agent:** Codex (`<model-id>`) — caro-frustrated-beta
...
**Agent:** Codex (`<model-id>`) — caro-frustrated-beta orchestrator
...
Append a single line to `.Codex/beta-testing/runs/INDEX.md` for trend
tracking:
...
1. `git add .Codex/beta-testing/runs/<date>/`
2. `git commit -m "chore(qa): frustrated-beta run <date>"` (on the
   automation branch — never on `main`, per `.Codex/rules/git-workflow.md`)
3. `git push`
...
| 3 | Schedule entry | `cat .Codex/automation/config/schedule.yaml` shows `frustrated_qa_loop` at `0 5 * * *` |
...
- **Never run on `main`.** Per `.Codex/rules/git-workflow.md`. The cron
...
- `.Codex/agents/caro-frustrated-beta.md` — the persona sub-agent
- `.Codex/skills/unbiased-beta-tester/SKILL.md` — the persona-framework root
- `.Codex/skills/beta-test-cycles/SKILL.md` — pattern-refinement loop
- `.Codex/commands/qa-automation-loop.md` — sister 9 AM multi-profile audit
- `.Codex/memory/qa_agent_role.md` — persistent QA identity & escalation
- `~/.Codex/rules/pr-comment-structure.md` — canonical comment shape
- `.Codex/rules/git-workflow.md` — feature-branch enforcement
