---
name: "source-command-ml-fine-tune-loop"
description: "Migrated source command `ml-fine-tune-loop`"
---

# source-command-ml-fine-tune-loop

Use this skill when the user asks to run the migrated source command `ml-fine-tune-loop`.

## Command Template

# ML Fine-Tune Loop

Autonomous daily routine for the **caro-ml-ds-engineer** role. Runs at 12 AM PST per `.Codex/automation/config/schedule.yaml`. One pass per 24h, capped.
...
- Refuse to run if a non-dry pass already ran in the last 23 hours (check `.Codex/memory/ml-session-log.md` last entry timestamp).
- Read `.Codex/agents/ml-ds-engineer.md` for the persona.
- Read `~/.Codex/projects/-Users-kobik-private-workspace-caro/memory/ml_ds_engineer_role.md` for the standing operational rules.
...
tail -40 .Codex/memory/ml-session-log.md
```
...
Append to `.Codex/memory/ml-session-log.md`:
...
The schedule layer surfaces this on the Slack/email channel configured in `.Codex/automation/config/schedule.yaml` (`notify_on_failure: true`, plus the loop's own `notify_on_completion` if the user opts in).
...
Inherits all from [.Codex/agents/ml-ds-engineer.md](.Codex/agents/ml-ds-engineer.md). Notably:
...
- Every issue/PR comment follows `~/.Codex/rules/pr-comment-structure.md`.

## See Also

- [.Codex/agents/ml-ds-engineer.md](.Codex/agents/ml-ds-engineer.md) — the agent this loop spawns
- [.Codex/commands/caro.ml.md](.Codex/commands/caro.ml.md) — interactive twin of this loop
- [.Codex/automation/config/schedule.yaml](.Codex/automation/config/schedule.yaml) — cron entry: `ml_fine_tune_loop @ 0 0 * * *`
- [.Codex/commands/qa-automation-loop.md](.Codex/commands/qa-automation-loop.md) — sibling loop for QA; same architectural pattern
