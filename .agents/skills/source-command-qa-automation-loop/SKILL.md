---
name: "source-command-qa-automation-loop"
description: "Migrated source command `qa-automation-loop`"
---

# source-command-qa-automation-loop

Use this skill when the user asks to run the migrated source command `qa-automation-loop`.
...
Load configuration from `.Codex/automation/config/qa_profiles.yaml`:
...
Generate run report at `.Codex/automation/state/qa_runs/{run_id}.yaml`
...
Edit `.Codex/automation/config/qa_profiles.yaml`:
...
Edit `.Codex/automation/config/qa_loop.yaml`:
...
- [QA_LOOP_DRS.md](../.Codex/automation/specs/QA_LOOP_DRS.md)
