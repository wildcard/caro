---
name: "source-command-social-queue"
description: "Migrated source command `social-queue`"
---

# source-command-social-queue

Use this skill when the user asks to run the migrated source command `social-queue`.
...
```yaml
# .Codex/automation/config/social_platforms.yaml
platforms:
  twitter:
    enabled: true
    max_chars: 280
...
```yaml
# .Codex/automation/config/social_queue.yaml
social_queue:
  approval:
    required: true
    auto_approve: false
...
```yaml
# .Codex/automation/queues/social_queue.yaml
queue:
...
- [SOCIAL_QUEUE_DRS.md](../.Codex/automation/specs/SOCIAL_QUEUE_DRS.md)
