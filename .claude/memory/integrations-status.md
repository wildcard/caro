...
>
> **Last updated:** 2026-09-12 (nightly pass, same PR) — **nothing has changed in 9 days, and the duplicate flood this PR exists to stop has produced four more.** ✓ VERIFIED 2026-09-12: crates.io `max_version` still **1.4.0 (2026-05-09)**; still no `v1.5.0` tag (only the two zero-asset drafts); `main` HEAD still `be07b22b` (2026-07-19). Since this PR was opened on 09-03, four more version-banner duplicates were filed — **#1434 (9/4), #1442 (9/7), #1444 (9/8), #1450 (9/12)** — the last one titled "(re-drift)". That is the predicted ~1-every-2-days rate, measured, while the two-line fix waits. Backend rows re-validated tonight (unchanged); the `caro-shell` skill row — the matrix's oldest at 2026-04-26 — is re-validated PASS and re-dated. Prior header follows.
>
> **2026-09-03 (nightly pass) — release still stalled; repo dormant.** ✓ VERIFIED: crates.io `max_version` is still **1.4.0 (2026-05-09)**, there is still **no `v1.5.0` tag** (only two zero-asset `v1.5.0` GitHub *drafts*), and `main` HEAD is `be07b22b` (2026-07-19) with no commits in ~6 weeks. The #1115 roster divergence therefore remains **live in the shipped binary** exactly as recorded on 2026-07-11 (`--backend-info` advertises `static`/`claude`; `--backend claude` → `Unknown backend`), and every backend merged since 2026-05-09 is still stranded. Tonight's PR does **not** touch backends — it fixes the `CLAUDE.md` version banner, whose ambiguity against `Cargo.toml`'s `1.5.0` has caused **~26 duplicate "version banner is stale" issues** (five of them — #1420/#1425/#1426/#1427/#1431 — filed in the twelve days *after* [#1419](https://github.com/wildcard/caro/issues/1419) documented the first 21). The release-cadence P0 stays open at #1419; this PR only stops the repo manufacturing a duplicate every other day while it waits.
...
| Tool | Status | Last validated | Method | GH | Notes |
|---|---|---|---|---|---|
| Claude Code skill (`caro-shell`) | ✅ working | 2026-09-12 | published 1.4.0, skill-documented invocation forms re-run verbatim: `caro --dry-run "clean up unused docker volumes"` → `docker system prune -f && docker volume prune -f` ✓; `caro --dry-run --explain "list pdf files"` → command + "Options explained" block ✓ | — | Was the oldest row in the matrix (2026-04-26, 4.5 months). Re-validated 2026-09-12: the skill's own v1.4.0 caveats are still exactly right — it already tells agents not to pass `--backend claude` / `--backend static`, and both are still rejected by the shipped binary. No drift; no fix needed. |
...
4. **Release-cadence P0 — [#1419](https://github.com/wildcard/caro/issues/1419) (filed 2026-08-21, re-validated 2026-08-30 and 2026-09-03; out of integrator hands — needs a human to push a tag):** `Cargo.toml`/`CHANGELOG.md` declare `1.5.0`, but no `v1.5.0` tag was ever pushed, so `publish.yml` never fired. crates.io is still `1.4.0` (2026-05-09, ~4 months). `main` compiles green, so nothing technical blocks the cut — it is a missed step 2 of `release-version-alignment.md`. Every OpenAI-shim / MCP-server night below is moot for real users until this lands.

5. **Duplicate-issue containment (2026-09-03 PR):** `CLAUDE.md`'s version banner now states published-vs-in-repo explicitly and points at #1419, to stop the ~26-and-counting "version banner is stale" duplicates. Mitigation only; #1419 is the real fix.
