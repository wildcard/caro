...
Check `.Codex/memory/current-tasks.md` for active work items.
...
- `.Codex/rules/constitution.md` - **Precedence index for all `.Codex/rules/` files (read first)**
- `src/safety/patterns.rs` - All dangerous command patterns
- `src/prompts/command_templates.rs` - LLM prompt templates
- `.Codex/skills/` - Domain expertise (load on-demand)
- `.Codex/commands/` - Workflow commands
- `.Codex/beta-testing/` - Test infrastructure
...
the beads backlog. Those are owned by Codex, Crush, and specialist agents.
