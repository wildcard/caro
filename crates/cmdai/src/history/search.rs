...
impl FTS5SearchEngine {
    ...
    pub async fn search(&mut self, query: &SearchQuery) -> Result<Vec<SearchResult>> {
        ...
        if let Some(filters) = &query.filters {
            if let Some(tags) = &filters.tags {
                let tag_conditions: Vec<String> =
                    tags.iter().map(|_| "h.tags LIKE ?".to_string()).collect();
        ...
        let mut query_builder = sqlx::query_as::<_, FTS5SearchRow>(&sql).bind(&fts_query);
        ...
        for row in rows {
            ...
            let entry = CommandHistoryEntry {
                ...
                user_input: Some(row.natural_language),
    ...
    fn build_fts_query(&self, text: &str) -> Result<String> {
        ...
        for (i, token) in tokens.iter().enumerate() {
            if token.starts_with('+') {
                ...
                // Wildcard
                query.push_str(&format!("{} ", token));
            } else if token.to_uppercase() == "AND"
                || token.to_uppercase() == "OR"
                || token.to_uppercase() == "NOT"
            {
                // Boolean operators
...
mod tests {
    ...
    async fn test_fts5_engine_initialization() {
        ...
        let result =
            sqlx::query("SELECT name FROM sqlite_master WHERE type='table' AND name='history_fts'")
                .fetch_optional(&pool)
                .await
                .unwrap();
