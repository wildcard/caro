...
pub enum SelectionStrategy {
    ...
    PreferredBackend {
        preferred: String,
        fallback: Box<SelectionStrategy>,
    },
}
...
impl PerformanceMonitor {
    ...
    async fn update_backend_metrics(
        ...
        backend_metrics.min_response_time = backend_metrics
            .min_response_time
            .min(snapshot.response_time);
        backend_metrics.max_response_time = backend_metrics
            .max_response_time
            .max(snapshot.response_time);
    ...
    pub async fn get_historical_data(
        ...
        let mut data: Vec<PerformanceSnapshot> = history
            .iter()
            .filter(|snapshot| backend_name.map_or(true, |name| snapshot.backend_name == name))
            .cloned()
            .collect();
    ...
    pub async fn select_backend(
        ...
        match strategy {
            SelectionStrategy::LowestLatency => available_backends
                .iter()
                .filter_map(|name| metrics.get(name).map(|m| (name, m)))
                .filter(|(_, m)| m.availability > 0.0)
                .min_by_key(|(_, m)| m.avg_response_time)
                .map(|(name, _)| name.clone()),
            SelectionStrategy::HighestReliability => available_backends
                .iter()
                .filter_map(|name| metrics.get(name).map(|m| (name, m)))
                .filter(|(_, m)| m.availability > 0.0)
                .max_by(|(_, a), (_, b)| a.success_rate.partial_cmp(&b.success_rate).unwrap())
            ...
            SelectionStrategy::Balanced {
                ...
                .max_by(|(_, a), (_, b)| {
                    let score_a =
                        self.calculate_balanced_score(a, *latency_weight, *reliability_weight);
                    let score_b =
                        self.calculate_balanced_score(b, *latency_weight, *reliability_weight);
            ...
            SelectionStrategy::RoundRobin => {
                ...
                let healthy_backends: Vec<_> = available_backends
                    .iter()
                    .filter(|name| metrics.get(*name).map_or(false, |m| m.availability > 0.0))
                    .collect();
            ...
            SelectionStrategy::PreferredBackend {
                preferred,
                fallback,
            } => {
                if available_backends.contains(preferred) {
                    let preferred_health = self.get_health_status(preferred).await;
                    if preferred_health == HealthStatus::Healthy
                        || preferred_health == HealthStatus::Degraded
                    {
...
impl MetricsCollector {
    ...
    pub async fn finish(
        ...
        monitor
            .record_measurement(&self.backend_name, duration, success, error_message)
            .await?;
...
mod tests {
    ...
    async fn test_record_measurement() {
        ...
        let result = monitor
            .record_measurement("test_backend", Duration::from_millis(150), true, None)
            .await;
    ...
    async fn test_backend_selection_lowest_latency() {
        ...
        monitor
            .record_measurement("fast", Duration::from_millis(50), true, None)
            .await
            .unwrap();
        monitor
            .record_measurement("slow", Duration::from_millis(200), true, None)
            .await
            .unwrap();

        let backends = vec!["fast".to_string(), "slow".to_string()];
        let selected = monitor
            .select_backend(&backends, &SelectionStrategy::LowestLatency)
            .await;
    ...
    async fn test_backend_selection_highest_reliability() {
        ...
        monitor
            .record_measurement("reliable", Duration::from_millis(100), true, None)
            .await
            .unwrap();
        monitor
            .record_measurement(
        ...
        let selected = monitor
            .select_backend(&backends, &SelectionStrategy::HighestReliability)
            .await;
    ...
    async fn test_health_status_calculation() {
        ...
        // Healthy backend
        monitor
            .record_measurement("healthy", Duration::from_millis(100), true, None)
            .await
            .unwrap();
    ...
    async fn test_balanced_selection_strategy() {
        ...
        monitor
            .record_measurement("fast_unreliable", Duration::from_millis(50), false, None)
            .await
            .unwrap();
        monitor
            .record_measurement("slow_reliable", Duration::from_millis(200), true, None)
            .await
            .unwrap();
