...
|---|---|---|---|
| ✅ | Budgets are part of the Task spec | `loop.sh` defaulted to `RALPH_MAX_ITERATIONS=0` (unlimited), with no per-iteration time limit | **Done in this PR:** default cap 20, per-iteration `RALPH_ITERATION_TIMEOUT` (30m), and a non-integer cap is rejected. Guard: `tests/harness/loop.test.sh`. Still open: `schedule.yaml` `timeout_minutes` is not enforced by anything in-repo |
