...
### Risks & Mitigations
- Cargo workspace conflicts → use separate Cargo.toml for evaluation harness
- Shellcheck not installed → document prerequisite in README
...
### Risks & Mitigations
- JSON schema drift → add JSON schema file for validation
- Large datasets slow loading → defer optimization until needed

---

## Work Package WP03: Command Correctness Evaluation (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- CLI invocation timeout → set 30s timeout per command
- Semantic equivalence too broad → start conservative, expand iteratively
- Dataset quality → manually review all test cases for correctness

---

## Work Package WP04: Safety Pattern Validation (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Safety module API changes → use stable public interface only
- False positive rate too high → document findings, defer fixes to safety module improvements
...
### Risks & Mitigations
- Shellcheck not installed → fail gracefully with clear error message, document prereq
- Shellcheck version differences → pin minimum version requirement (0.8.0+)
...
### Risks & Mitigations
- Backends not configured → skip unavailable backends with warning
- Statistical power low → require minimum 30 samples per backend for significance tests
...
### Risks & Mitigations
- Report file size too large → limit detailed output, use summary aggregations
- Markdown formatting breaks in different viewers → use GitHub-flavored markdown standard
...
### Phase 2: MVP Evaluation (WP03-WP04) 🎯
...
This delivers:
- ✅ Command correctness validation (User Story 1, P1)
- ✅ Safety pattern detection accuracy (User Story 2, P1)
- ✅ Automated test execution infrastructure
- ✅ Basic JSON reporting
- ✅ Success criteria SC-001, SC-002, SC-003, SC-006 validated
