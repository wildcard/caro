...
pub async fn handle_telemetry(cmd: TelemetryCommands, storage_path: PathBuf) -> Result<()> {
    match cmd {
        TelemetryCommands::Show { limit, session } => show_events(storage_path, limit, session).await,
...
async fn show_events(storage_path: PathBuf, limit: usize, session: Option<String>) -> Result<()> {
    let storage = TelemetryStorage::new(storage_path)
        .context("Failed to open telemetry storage")?;
    ...
    println!();
    println!("{}", format!("Showing {} most recent events:", events.len()).bright_white().bold());
    println!();

    for (i, event) in events.iter().enumerate() {
        let event_num = format!("[{}]", i + 1).bright_blue();
        let timestamp = event.timestamp.format("%Y-%m-%d %H:%M:%S").to_string().bright_black();
        let session = format!("Session: {}", event.session_id.as_str()).bright_black();

        println!("{} {} {}", event_num, timestamp, session);

        match &event.event_type {
            crate::telemetry::EventType::SessionStart { version, platform, shell_type, backend_available } => {
                println!("  {} {}", "Type:".bright_white(), "SessionStart".green());
                println!("  {} v{}", "Version:".bright_white(), version);
                println!("  {} {} ({})", "Platform:".bright_white(), platform, shell_type);
                println!("  {} {}", "Backends:".bright_white(), backend_available.join(", "));
            }
            crate::telemetry::EventType::SessionEnd { duration_ms, commands_generated, commands_executed } => {
                println!("  {} {}", "Type:".bright_white(), "SessionEnd".yellow());
                println!("  {} {}ms", "Duration:".bright_white(), duration_ms);
                println!("  {} {} generated, {} executed", "Commands:".bright_white(), commands_generated, commands_executed);
            }
            crate::telemetry::EventType::CommandGeneration { backend, duration_ms, success, error_category } => {
                let status = if *success { "Success".green() } else { "Failed".red() };
                println!("  {} {} ({})", "Type:".bright_white(), "CommandGeneration".cyan(), status);
            ...
            crate::telemetry::EventType::SafetyValidation { risk_level, action_taken, pattern_category } => {
                ...
                println!("  {} {} ({})", "Type:".bright_white(), "SafetyValidation".magenta(), action_color);
                println!("  {} {}", "Risk:".bright_white(), risk_level);
                if let Some(category) = pattern_category {
                    println!("  {} {}", "Category:".bright_white(), category);
                }
            }
            crate::telemetry::EventType::BackendError { backend, error_category, recoverable } => {
                let recovery = if *recoverable { "Recoverable".yellow() } else { "Fatal".red() };
                println!("  {} {} ({})", "Type:".bright_white(), "BackendError".red(), recovery);
                println!("  {} {}", "Backend:".bright_white(), backend);
    ...
    println!("{}", format!("Total: {} events", events.len()).bright_black());
    println!();
...
async fn clear_events(storage_path: PathBuf, force: bool) -> Result<()> {
    ...
    println!();
    println!("{}", format!("✓ Deleted {} telemetry events", count).green());
    println!();
...
async fn show_status(storage_path: PathBuf) -> Result<()> {
    ...
    let storage = TelemetryStorage::new(storage_path.clone())
        .context("Failed to open telemetry storage")?;
    ...
    if !config.air_gapped {
        println!("  {} {}", "Endpoint:".bright_white(), config.endpoint.bright_black());
    }
    ...
    // Storage info
    println!("  {} {}", "Storage:".bright_white(), storage_path.display().to_string().bright_black());
    ...
    println!("  {} Disable telemetry", "caro config set telemetry.enabled false".cyan());
    println!("  {} View queued events", "caro telemetry show".cyan());
    println!("  {} Export data (air-gapped)", "caro telemetry export".cyan());
