...
impl ConfigIO {
    ...
    pub async fn export(
        ...
        info!(
            "Configuration exported to {}: {} bytes",
            path.display(),
            result.size_bytes
        );
        Ok(result)
    ...
    pub async fn import(&self, path: &Path, format: Option<ImportFormat>) -> Result<ImportResult> {
        let start = std::time::Instant::now();
        ...
        if result.validation_report.is_valid() {
            ...
            warn!(
                ...
                result.validation_report.warnings.len()
            );
        }
    ...
    fn export_shell_env(&self, config: &ConfigurationState) -> Result<String> {
        ...
        env_vars.push(format!(
            "export CMDAI_BACKEND=\"{}\"",
            config.preferred_backend
        ));
        env_vars.push(format!(
            "export CMDAI_HISTORY_ENABLED=\"{}\"",
    ...
    fn auto_parse(&self, content: &str) -> Result<ConfigurationState> {
        ...
        for converter in &self.converters {
            if let Ok(config) = converter.deserialize(content) {
                debug!(
                    "Successfully parsed configuration as {}",
    ...
    fn apply_migrations(&self, mut config: ConfigurationState) -> Result<ConfigurationState> {
        ...
        if config.fallback_chain.is_empty() {
            config.fallback_chain = vec![
                crate::models::BackendType::VLlm,
                crate::models::BackendType::Ollama,
                crate::models::BackendType::Auto,
            ];
    ...
    fn merge_prefer_overlay(
        ...
        merged
            .backend_configs
            .extend(overlay.backend_configs.clone());
        ...
        merged
            ...
            .extend(overlay.custom_safety_patterns.clone());
...
impl FormatConverter for TomlConverter {
    ...
    fn deserialize(&self, content: &str) -> Result<ConfigurationState> {
        toml::from_str(content).context("Failed to deserialize configuration from TOML")
    }
...
impl FormatConverter for JsonConverter {
    ...
    fn deserialize(&self, content: &str) -> Result<ConfigurationState> {
        serde_json::from_str(content).context("Failed to deserialize configuration from JSON")
    }
...
impl FormatConverter for YamlConverter {
    ...
    fn deserialize(&self, content: &str) -> Result<ConfigurationState> {
        serde_yaml::from_str(content).context("Failed to deserialize configuration from YAML")
    }
...
mod tests {
    ...
    async fn test_export_import_roundtrip() {
        ...
        let export_result = io
            .export(&config, &config_path, ExportFormat::Toml)
            .await
            .unwrap();
        ...
        assert_eq!(
            config.preferred_backend,
            import_result.config.preferred_backend
        );
    }
    ...
    fn test_merge_strategies() {
        ...
        // Test replace all
        let merged = io
            .merge(&base, &overlay, MergeStrategy::ReplaceAll)
            .unwrap();
        ...
        // Test prefer overlay
        let merged = io
            .merge(&base, &overlay, MergeStrategy::PreferOverlay)
            .unwrap();
        assert_eq!(merged.history_enabled, overlay.history_enabled);

        // Test prefer base
        let merged = io
            .merge(&base, &overlay, MergeStrategy::PreferBase)
            .unwrap();
        assert_eq!(merged.auto_cleanup_days, overlay.auto_cleanup_days);
    }
}
