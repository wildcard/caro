...
impl std::fmt::Display for PrivacyLevel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PrivacyLevel::Basic => write!(f, "Basic"),
            PrivacyLevel::Strict => write!(f, "Strict"),
            PrivacyLevel::Paranoid => write!(f, "Paranoid"),
            PrivacyLevel::Telemetry => write!(f, "Telemetry"),
        }
    }
}
...
impl ConfigurationState {
    ...
    pub fn update_backend_config(&mut self, backend_name: &str, config: BackendConfig) {
        self.backend_configs
            .insert(backend_name.to_string(), config);
    }
...
mod tests {
    ...
    fn test_toml_serialization() {
        ...
        let deserialized =
            ConfigurationState::from_toml(&toml_str).expect("Should deserialize from TOML");
    ...
    fn test_custom_safety_patterns() {
        ...
        // Add valid pattern
        assert!(config
            .add_custom_safety_pattern("rm -rf".to_string())
            .is_ok());
