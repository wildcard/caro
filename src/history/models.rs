...
impl ExecutionMetadata {
    ...
    pub fn with_execution_result(mut self, exit_code: i32, execution_time: Duration, output_size: usize) -> Self {
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.confidence_score < 0.0 || self.confidence_score > 1.0 {
            return Err(format!("Confidence score must be between 0.0 and 1.0, got {}", self.confidence_score));
        }
...
impl SafetyMetadata {
    ...
    pub fn get_risk_description(&self) -> String {
        match self.risk_level {
            ...
            RiskLevel::Critical => format!("Critical risk - dangerous command (score: {:.2})", self.safety_score),
        }
    }
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.safety_score < 0.0 || self.safety_score > 1.0 {
            return Err(format!("Safety score must be between 0.0 and 1.0, got {}", self.safety_score));
        }
...
mod tests {
    ...
    fn test_execution_metadata_total_processing_time() {
        ...
        assert_eq!(metadata.total_processing_time(), Duration::from_millis(1550));
    }
    ...
    fn test_safety_metadata_creation() {
        let metadata = SafetyMetadata::new(
            RiskLevel::Moderate,
            0.45,
    ...
    fn test_safety_metadata_descriptions() {
        let safe_metadata = SafetyMetadata::new(RiskLevel::Safe, 0.15, SafetyLevel::Moderate);
        assert!(safe_metadata.get_risk_description().contains("Safe command"));
        
        let critical_metadata = SafetyMetadata::new(RiskLevel::Critical, 0.95, SafetyLevel::Strict);
        assert!(critical_metadata.get_risk_description().contains("Critical risk"));
    }
    ...
    fn test_command_history_entry_with_extended_metadata() {
        ...
        let safety_metadata = SafetyMetadata::new(
            RiskLevel::Safe,
            0.15,
            SafetyLevel::Moderate,
        );
