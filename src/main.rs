...
#[command(long_about = "cmdai converts natural language descriptions into safe POSIX shell commands using local language models. Features safety validation, multiple output formats, and configurable backends.")]
#[command(version)]
struct Cli {
    ...
    /// Target shell type
    #[arg(short, long, help = "Shell type (bash, zsh, fish, sh, powershell, cmd)")]
    shell: Option<String>,
    ...
    #[arg(short = 'y', long, help = "Auto-confirm dangerous commands without prompting")]
    confirm: bool,
...
async fn print_plain_output(result: &cmdai::cli::CliResult, cli: &Cli) -> Result<(), CliError> {
    ...
    if result.requires_confirmation && !cli.confirm {
        ...
        if atty::is(atty::Stream::Stdin) {
            ...
            println!("{}", "✓ Confirmed. Proceeding with command execution.".green());
        } else {
...
async fn show_configuration(cli: &Cli) -> Result<String, CliError> {
    let config_manager = if let Some(config_file) = &cli.config_file {
        ConfigManager::with_config_path(config_file.into()).map_err(|e| CliError::ConfigurationError {
