...
async fn main() {
    ...
    // Handle --suggest or missing prompt (enter interactive suggestions mode)
    if cli.suggest || cli.prompt.is_none() {
...
async fn show_suggestions(cli: &Cli) -> Result<Option<String>, CliError> {
    ...
    if !atty::is(atty::Stream::Stdin) {
        ...
        println!();
        println!("{}", "Run with a prompt: cmdai \"your query here\"".dimmed());
        return Ok(None);
    }
