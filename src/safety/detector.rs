...
impl DangerousPatternDetector {
    ...
    pub fn detect(&mut self, command: &str) -> Result<DetectionResult> {
        ...
        self.metrics.record_detection(start.elapsed(), result.is_dangerous);
    ...
    fn check_critical_patterns(&self, command: &str) -> Result<Option<DetectionResult>> {
        ...
        if self.patterns.fork_bomb.is_match(command) {
            return Ok(Some(DetectionResult {
                ...
                suggestions: vec!["This command is a fork bomb and will crash your system. Do not run it.".to_string()],
                confidence: 1.0,
            }));
        }
        ...
        if self.patterns.system_destruction.is_match(command) {
            return Ok(Some(DetectionResult {
                ...
                command: command.to_string(),
                normalized_command: command.to_string(),
                suggestions: vec!["This command will destroy critical system files. Do not run it.".to_string()],
                confidence: 1.0,
            }));
        }
    ...
    fn decode_escaped_strings(&self, input: &str) -> String {
        ...
        decoded = urlencoding::decode(&decoded).unwrap_or(decoded.clone()).to_string();
    ...
    fn analyze_context(&self, command: &str, patterns: &[PatternInfo]) -> Result<RiskLevel> {
        ...
        if (command.contains(">") || command.contains(">>")) &&
           (command.contains("/etc/") || command.contains("/sys/") || command.contains("/boot/")) {
            risk = risk.max(RiskLevel::High);
        }
        ...
        if (command.contains("&&") || command.contains(";") || command.contains("|")) && !patterns.is_empty() {
    ...
    fn check_pattern_combinations(&self, patterns: &[PatternInfo]) -> RiskLevel {
        ...
        let high_risk_count = patterns.iter().filter(|p| p.risk_level >= RiskLevel::High).count();
        if high_risk_count >= 2 {
            return RiskLevel::High;
        }
        
        patterns.iter().map(|p| p.risk_level).max().unwrap_or(RiskLevel::Low)
    }
        ...
        for pattern in patterns {
            match pattern.category {
                PatternCategory::FileDestruction => {
                    ...
                    suggestions.push("Add '--interactive' flag to confirm each deletion".to_string());
                }
                ...
                PatternCategory::NetworkDanger => {
                    ...
                    suggestions.push("Consider using rate limiting or connection limits".to_string());
                }
                PatternCategory::DataLoss => {
                    suggestions.push("Create a backup before proceeding".to_string());
                    suggestions.push("Use '--dry-run' flag if available to preview changes".to_string());
                }
                _ => {}
            }
        }
...
impl CompiledPatterns {
    ...
    pub fn production() -> Self {
        ...
        dangerous.push((
            ...
            PatternInfo::new("Recursive root deletion", "Attempts to delete from root", RiskLevel::Critical, PatternCategory::FileDestruction),
        ));
        
        dangerous.push((
            Regex::new(r"rm\s+(-rf?|--recursive)\s+(\$HOME|~)/?$").unwrap(),
            PatternInfo::new("Home directory deletion", "Attempts to delete entire home directory", RiskLevel::Critical, PatternCategory::FileDestruction),
        ));
        
        dangerous.push((
            Regex::new(r"find\s+.*-delete").unwrap(),
            PatternInfo::new("Find with delete", "Find command with deletion", RiskLevel::Medium, PatternCategory::FileDestruction),
        ));
        
        // System modification patterns
        dangerous.push((
            Regex::new(r"chmod\s+(777|-R\s+777)").unwrap(),
            PatternInfo::new("Dangerous permissions", "Setting world-writable permissions", RiskLevel::High, PatternCategory::SystemModification),
        ));
        
        dangerous.push((
            Regex::new(r"chown\s+-R\s+(root|0):").unwrap(),
            PatternInfo::new("Recursive root ownership", "Changing ownership to root recursively", RiskLevel::High, PatternCategory::SystemModification),
        ));
        
        // Disk operations
        dangerous.push((
            Regex::new(r"dd\s+.*of=/dev/(sd|hd|nvme)").unwrap(),
            PatternInfo::new("Direct disk write", "Writing directly to disk device", RiskLevel::Critical, PatternCategory::DiskOperation),
        ));
        
        dangerous.push((
            Regex::new(r"mkfs\.").unwrap(),
            PatternInfo::new("Filesystem creation", "Creating new filesystem (will destroy data)", RiskLevel::Critical, PatternCategory::DiskOperation),
        ));
        
        // Network dangers
        dangerous.push((
            Regex::new(r"curl.*\|\s*(bash|sh)").unwrap(),
            PatternInfo::new("Curl pipe to shell", "Downloading and executing remote script", RiskLevel::High, PatternCategory::NetworkDanger),
        ));
        
        dangerous.push((
            Regex::new(r"wget.*\|\s*(bash|sh)").unwrap(),
            PatternInfo::new("Wget pipe to shell", "Downloading and executing remote script", RiskLevel::High, PatternCategory::NetworkDanger),
        ));
        
        // Process operations
        dangerous.push((
            Regex::new(r"kill(all)?\s+-9").unwrap(),
            PatternInfo::new("Force kill", "Force killing processes", RiskLevel::Medium, PatternCategory::ProcessControl),
        ));
        
        dangerous.push((
            Regex::new(r"pkill\s+-9").unwrap(),
            PatternInfo::new("Pattern kill", "Force killing processes by pattern", RiskLevel::Medium, PatternCategory::ProcessControl),
        ));
...
impl PatternInfo {
    pub fn new(name: &str, description: &str, risk_level: RiskLevel, category: PatternCategory) -> Self {
        Self {
            name: name.to_string(),
...
impl DetectionResult {
    ...
    pub fn to_safety_assessment(&self) -> SafetyAssessment {
        SafetyAssessment {
            ...
            warnings: self.detected_patterns.iter()
...
mod tests {
    ...
    fn test_safe_commands() {
        ...
        for cmd in safe_commands {
            ...
            assert!(result.risk_level <= RiskLevel::Low, "Incorrectly flagged: {}", cmd);
        }
    }
    ...
    fn test_metrics_recording() {
        ...
        assert_eq!(metrics.avg_detection_time, std::time::Duration::from_millis(15));
