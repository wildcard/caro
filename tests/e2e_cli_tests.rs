...
fn e2e_natural_language_with_flags() {
    ...
    let test_cases = [
        ...
        // output format flag
        (
            &["--output", "json", "show", "disk", "usage"][..],
            "--output",
        ),
        // safety flag (use a safe command to avoid blocking)
        (
            &["--safety", "moderate", "show", "temp", "directory"][..],
            "--safety",
        ),
        // verbose flag
...
fn e2e_output_equivalence_quoted_vs_unquoted() {
    ...
    let unquoted_output =
        runner.run_success(&["list", "all", "files", "in", "current", "directory"]);
...
fn e2e_special_characters_in_natural_language() {
    ...
    for args in &test_cases {
        ...
        assert!(!result.stdout.is_empty(), "No output for args: {:?}", args);
    }
