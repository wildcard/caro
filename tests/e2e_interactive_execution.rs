...
fn test_e2e_execute_flag_runs_command() {
    ...
    assert!(
        stdout.contains("caro") || stdout.contains("/home/") || stdout.contains("/"),
...
fn test_e2e_json_output_format() {
    ...
    let (stdout, _stderr, exit_code) =
        run_caro(&["--output", "json", "--execute", "current directory"]);
...
fn test_e2e_version_flag() {
    ...
    // Should show version
    assert!(
        stdout.contains("caro") && (stdout.contains("0.") || stdout.contains("1.")),
