...
async fn smoke_test_basic_inference() {
    ...
    assert!(cmd.command.len() < 500, "Command should be reasonable length");
}
...
async fn smoke_test_determinism() {
    ...
    let result1 = backend.generate_command(&request).await.expect("Should succeed");
    let result2 = backend.generate_command(&request).await.expect("Should succeed");
    ...
    let overlap = if total > 0 { (common as f64 / total as f64) * 100.0 } else { 0.0 };
    
    assert!(overlap >= 70.0, "Should be reasonably deterministic, got {:.1}% overlap", overlap);
}
...
async fn smoke_test_output_structure() {
    ...
    for (prompt, shell) in cases {
        ...
        let result = backend.generate_command(&request).await.expect("Should succeed");
...
async fn smoke_test_performance() {
    ...
    assert!(load_time.as_secs() < 60, "Load should take <60s, took {:?}", load_time);
    
    let request = CommandRequest::new("list files", ShellType::Bash);
    
    // Cold inference
    let start = std::time::Instant::now();
    let _result = backend.generate_command(&request).await.expect("Should succeed");
    let cold_time = start.elapsed();
    
    assert!(cold_time.as_secs() < 30, "Cold inference should take <30s, took {:?}", cold_time);
    ...
    assert!(warm_time.as_secs() < 10, "Warm inference should take <10s, took {:?}", warm_time);
}
