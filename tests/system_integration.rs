...
async fn test_e2e_embedded_workflow() {
    ...
    for test_case in E2E_TEST_CASES {
        ...
        assert!(result.is_ok(), "Failed to generate command for case '{}': {:?}", 
                test_case.name, result.err());
        
        let command = result.unwrap();
        
        // Verify timing requirements
        assert!(generation_time.as_millis() <= test_case.max_generation_time_ms as u128,
                "Generation took too long for case '{}': {}ms > {}ms",
                test_case.name, generation_time.as_millis(), test_case.max_generation_time_ms);
        
        // Verify command is not empty
        assert!(!command.command.is_empty(), 
                "Generated empty command for case '{}'", test_case.name);
        ...
        if let Some(should_contain) = test_case.should_contain {
            ...
            if !contains_expected && !is_safe_fallback {
                panic!("Command '{}' should contain '{}' or be a safe fallback for case '{}'",
        ...
        if let Some(should_not_contain) = test_case.should_not_contain {
            assert!(!command.command.contains(should_not_contain),
                    "Command '{}' should not contain '{}' for case '{}'",
                    command.command, should_not_contain, test_case.name);
        }
        
        // Verify safety level matches expectation
        if test_case.expected_safe {
            assert_ne!(command.safety_level, RiskLevel::Critical,
                      "Safe command '{}' should not be marked as critical for case '{}'",
                      command.command, test_case.name);
        }
        ...
        println!("✓ Case '{}' passed - Command: {}", test_case.name, command.command);
    }
}
...
async fn test_backend_selection_and_fallback() {
    ...
    let command = result.unwrap();
    assert!(command.backend_used.to_lowercase().contains("embedded") || command.backend_used.contains("qwen"));
...
async fn test_shell_type_integration() {
    let backend = create_test_embedded_backend();
    let test_shells = vec![ShellType::Bash, ShellType::Zsh, ShellType::Fish, ShellType::Sh];
...
async fn test_safety_validation_integration() {
    ...
    for safe_request in safe_requests {
        ...
        assert!(result.is_ok(), "Safe request should succeed: {}", safe_request);
        let command = result.unwrap();
    ...
    for dangerous_request in dangerous_requests {
        ...
        assert!(result.is_ok(), "Dangerous request should get safe alternative: {}", dangerous_request);
        let command = result.unwrap();
        // Should get a safe echo command instead
        assert!(command.command.contains("echo"), 
...
async fn test_concurrent_request_handling() {
    ...
    for i in 0..num_concurrent {
        ...
        let handle = tokio::spawn(async move {
            let request = CommandRequest::new(
                &format!("list files in directory {}", i),
                ShellType::Bash
            );
    ...
    println!("✓ All {} concurrent requests completed successfully", num_concurrent);
}
    ...
    for request_text in &test_requests {
        ...
        if result.is_ok() {
            ...
            assert!(duration.as_millis() < 3000, 
                    "Request '{}' took too long: {}ms", request_text, duration.as_millis());
        }
    }
    
    if successful_generations > 0 {
        let avg_time = total_time / successful_generations as u32;
        println!("✓ Average generation time: {}ms over {} requests", 
                avg_time.as_millis(), successful_generations);
        
        // Average should be reasonable
        assert!(avg_time.as_millis() < 2000, 
                "Average generation time too high: {}ms", avg_time.as_millis());
    }
}
...
async fn test_system_integration_scenarios() {
    ...
    for (scenario, prompt) in dev_workflow {
        ...
        let result = backend.generate_command(&request).await;
        assert!(result.is_ok(), "Development scenario '{}' should work", scenario);
