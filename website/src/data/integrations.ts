...
export const outboundIntegrations: Integration[] = [
  {
    id: 'claude-code-skill-shell',
    name: 'Claude Code (caro-shell skill)',
    category: 'agent',
    status: 'partial',
    surface: 'claude-code-skill',
    lastValidated: '2026-05-16',
    description:
      'Skill source at `.claude/skills/caro-shell/SKILL.md` in this repo. Triggers when the user asks for shell-command synthesis; shells out to the published `caro` binary with `--dry-run` and presents the safety-validated suggestion for approval. ⚠️ Not installable via `caro skill install` in v1.4.0 — that subcommand bundles `caro-scaffold` instead (see next row). To use this skill, copy it into a project\'s `.claude/skills/` manually or have Claude Code auto-discover it from a checked-out caro repo.',
    snippet: [
      '# Install the caro CLI first',
      'cargo install caro',
      '',
      '# Then make caro-shell available to Claude Code. Two options:',
      '',
      '# Option A — auto-discover from a caro checkout',
      'git clone https://github.com/wildcard/caro ~/src/caro',
      'cd ~/src/caro    # Claude Code auto-discovers .claude/skills/',
      '',
      '# Option B — copy into your project',
      'mkdir -p .claude/skills/caro-shell',
      'curl -sSL https://raw.githubusercontent.com/wildcard/caro/main/.claude/skills/caro-shell/SKILL.md \\',
      '  -o .claude/skills/caro-shell/SKILL.md',
    ].join('\n'),
    homepage: 'https://www.anthropic.com/claude-code',
    tracking: 'roadmapped: have `caro skill install` learn to install caro-shell too',
  },
  {
    id: 'claude-code-skill-scaffold',
    name: 'Claude Code (caro-scaffold skill, bundled in v1.4.0)',
    category: 'agent',
    status: 'working',
    surface: 'claude-code-skill',
    lastValidated: '2026-05-16',
    description:
      'CaroML scaffolding skill bundled with the published `caro` binary. Installs to `~/.claude/skills/caro-scaffold/` via `caro skill install`. Triggers when the user wants to scaffold a `.caro` task file ("make a CaroML task that…", "scaffold a runbook for…", "/caro new <name>"). Distinct from `caro-shell` above — this one is for *creating repeatable tasks*, not for one-shot command synthesis.',
    snippet: [
      '# Install the caro CLI',
      'cargo install caro',
      '',
      ...
      '',
      '# Verify installation',
      'ls ~/.claude/skills/caro-scaffold/',
      '',
      '# Uninstall later if needed',
      'caro skill uninstall',
    ].join('\n'),
    homepage: 'https://www.anthropic.com/claude-code',
  },
  {
